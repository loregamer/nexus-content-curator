...
(function () {
  ...
  // Add CSS to hide mod tiles by default unless they have content-processed attribute
    // Define the custom CSS for /games/* pages
  const customCSS = `
  ...
  const injectStyles = () => {
    ...
    // Check if we are on a /games/* page and append the custom CSS if so
    if (window.location.href.startsWith("https://www.nexusmods.com/games/")) {
        finalCSS += `
