...
[/code]`;
  ...
  function addReportButtonToProfile() {
    ...
    // Add "Report all mods" button if we're on next.nexusmods.com
    if (window.location.href.includes("next.nexusmods.com/profile/")) {
      // Check if the button already exists
      if (buttonContainer.querySelector(".report-all-mods-button")) return;
      
      // Create and add the "Report all mods" button
      const reportAllModsButton = createReportAllModsButton();
      buttonContainer.appendChild(reportAllModsButton);
    }
  }

  // Function to create "Report all mods" button
  function createReportAllModsButton() {
    const button = document.createElement("button");
    button.className = "author-report-button report-all-mods-button";
    button.type = "button";
    button.style.backgroundColor = "#8B0000"; // Darker red to distinguish from author report
    ...
    button.innerHTML = `
      <span>
        ...
        <span class="typography-body-lg grow text-left leading-5">Report All Mods</span>
      </span>
    `;

    button.addEventListener("click", showReportAllModsForm);
    return button;
  }

  // Function to gather all mods from the profile page
  function getAllModsFromProfile() {
    const mods = [];
    const modTiles = document.querySelectorAll('.mod-tile');
    
    modTiles.forEach(tile => {
      const titleLink = tile.querySelector('a[href*="/mods/"]');
      if (!titleLink) return;
  ...
  // Function to create report all mods form HTML
  function createReportAllModsFormHTML() {
    return `
      <div class="form-overlay">
        <div class="mod-report-form report-all-mods-form" style="width: 600px; max-width: 90vw;">
          <button class="close">&times;</button>
          <h2>Report All Mods</h2>
          
          <div class="form-group">
            <label>Status</label>
            <select id="allModsStatus">
              <option value="BROKEN">Broken</option>
              <option value="LAME">Sucks</option>
              <option value="INFORMATIVE">Informative</option>
            </select>
          ...
          <div class="form-group">
            <label>Reason (optional - will be applied to all mods)</label>
            <textarea id="allModsReason" rows="3"></textarea>
          </div>
          
          <div class="form-group">
            <label>Alternative Mod Link (optional - will be applied to all mods)</label>
            <input type="text" id="allModsAlternative">
          </div>
          
          <div class="form-group">
            <label>Mods to be reported (<span id="modCount">0</span> found)</label>
            <div id="modsList" style="max-height: 200px; overflow-y: auto; background: #333; padding: 10px; border-radius: 4px; margin-top: 5px;">
              <p style="color: #999; text-align: center;">Loading mods...</p>
            </div>
          </div>
          
          <div class="buttons">
            <button class="primary" id="copyAllModsToClipboard">Copy to Clipboard</button>
            <button id="closeForm">Close</button>
          </div>
        </div>
      </div>
  ...
  // Function to show report all mods form
  function showReportAllModsForm() {
    // Create and add form if it doesn't exist
    if (!document.querySelector(".form-overlay")) {
      document.body.insertAdjacentHTML("beforeend", createReportAllModsFormHTML());
      ...
      document
        ...
        .addEventListener("click", hideReportForm);
      document
        .querySelector("#copyAllModsToClipboard")
        .addEventListener("click", copyAllModsToClipboard);
    ...
    // Populate mods list
    const mods = getAllModsFromProfile();
    const modsListContainer = document.querySelector("#modsList");
    const modCountElement = document.querySelector("#modCount");
    
    if (mods.length === 0) {
      modsListContainer.innerHTML = `<p style="color: #999; text-align: center;">No mods found on this profile.</p>`;
      modCountElement.textContent = "0";
    } else {
      modCountElement.textContent = mods.length;
      
      const modsList = mods.map(mod => `
        <div style="padding: 5px; border-bottom: 1px solid #444; display: flex; justify-content: space-between;">
          <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 70%;">${mod.title}</span>
          <span style="color: #999;">${mod.gameId}</span>
        </div>
      `).join('');
      
      modsListContainer.innerHTML = modsList;
    }
  }

  // Function to copy all mods to clipboard
  function copyAllModsToClipboard() {
    const mods = getAllModsFromProfile();
    if (mods.length === 0) {
      alert("No mods found to report!");
      return;
    }
    
    const status = document.querySelector("#allModsStatus").value;
    const reason = document.querySelector("#allModsReason").value;
    const alternative = document.querySelector("#allModsAlternative").value;
    
    // Get the username from the URL
    const urlParts = window.location.pathname.split("/");
    const usernameIndex = urlParts.indexOf("profile") + 1;
    const username = usernameIndex < urlParts.length ? urlParts[usernameIndex] : "Unknown";
    
    // Create BBCode formatted message with list of all mods
    let bbCodeMessage = `[b]Mod Report for ${username}:[/b]\n\n`;
    ...
    // Add each mod to the BBCode message and JSON data
    mods.forEach(mod => {
      bbCodeMessage += `[b]Mod:[/b] [url=https://www.nexusmods.com/${mod.gameId}/mods/${mod.modId}]${mod.title}[/url]\n`;
      bbCodeMessage += `[list]\n`;
      bbCodeMessage += `[*] [size=85][b]Game:[/b] ${mod.gameId}[/size]\n`;
      bbCodeMessage += `[*] [size=85][b]Status:[/b] ${status}[/size]\n`;
      
      if (reason) {
        bbCodeMessage += `[*] [size=85][b]Reason:[/b] ${reason}[/size]\n`;
      }
      
      if (alternative) {
        bbCodeMessage += `[*] [size=85][b]Alternative:[/b] ${alternative}[/size]\n`;
      }
      
      bbCodeMessage += `[/list]\n\n`;
      
      // Add to JSON data
      if (!jsonData["Mod Statuses"][mod.gameId]) {
        jsonData["Mod Statuses"][mod.gameId] = {};
      }
      
      if (!jsonData["Mod Statuses"][mod.gameId][status]) {
        jsonData["Mod Statuses"][mod.gameId][status] = [];
      }
      
      jsonData["Mod Statuses"][mod.gameId][status].push(mod.modId);
      
      if (reason || alternative) {
        if (!jsonData["Mod Descriptors"][mod.gameId]) {
          jsonData["Mod Descriptors"][mod.gameId] = {};
        }
        
        jsonData["Mod Descriptors"][mod.gameId][mod.modId] = {
          reason: reason || undefined,
          alternative: alternative || undefined
        };
      }
    ...
    navigator.clipboard
      ...
      .then(() => {
        // Store JSON data in localStorage for later use
        localStorage.setItem("lastAllModsReportJson", JSON.stringify(jsonData));
  ...
  // Function to setup label details toggle
